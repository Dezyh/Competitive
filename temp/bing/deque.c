...
void iterative_reverse(Deque *deque) {
  ...
  while (current != NULL){
    ...
    // Don't neeed to change the head_node if the deque is of size 0 or 1
    if (temp != NULL) {
      deque->top = temp->prev;
    }
...
//  [1 2 3] > [3, 2, 1]
// 1. [1] [2 3]
// 2. Calls recursive_reverse
// 3. [2] [3]
// 4. Calls recursive_reverse
// 5. [3] [ ]
// 6. Deque is now empty: 
  // if deque is empty; we are done reversing
   if (deque->size == 0){
   }
    else {
...
void split_deque(Deque *deque, int k) {
  Deque *large_d = new_deque();
  Deque *small_d = new_deque();
  int current_data;
  int current_large;
  int current_small;
  while (deque->size != 0){
    current_data = deque_pop(deque);
    if (current_data >= k){
      deque_insert(large_d, current_data);
    }
    else{
      deque_insert(small_d, current_data);
    }
  }
    // pop bottom of large_deque and insert at top of deque
  while (large_d->size !=0){
    current_large = deque_remove(large_d);
    deque_push(deque, current_large);
  }
  // pop top of small_deque and insert at bottom of deque
  while (small_d->size !=0){
    current_small = deque_pop(small_d);
    deque_insert(deque, current_small);
  }
  free_deque(small_d);
  free_deque(large_d);
}
